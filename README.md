...
WolfPro is a competitive-multiplayer version of **Return to Castle
Wolfenstein** (id Tech 3 lineage), built from id Software's original
RTCW MP GPL source release (August 12, 2010). It modernizes the engine
and adds tooling for competitive play: referee mode, match/stats
infrastructure, hit registration and hitbox work, cvar restrictions,
and antilag/unlag.

## What's different from vanilla RTCW

- **Engine**: native x64 builds, a Vulkan renderer alongside the classic
  OpenGL one, a native Linux/SDL2 client, CMake build, curl-based
  downloads.
...
- **Hit registration**: server-side skeletal-model-derived hitbox
  attachment.
...
- **Rendering**: widescreen HUD support, a realtime CPU/GPU profiler,
  and a TrueType-font-backed HUD/console character set as an
  alternative to the original pixel-art font.
...
- `src/qcommon/`, `src/client/`, `src/server/` — shared engine components,
...
- `src/game/omnibot/` — the Omni-bot integration.
