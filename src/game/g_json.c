...
void BuildPlayerStats(gclient_t *client, qboolean clientDisconnected)
{
    ...
    if (clientDisconnected && client != NULL)
    {
        ...
        if (level.disconnectStats[dc].guid == NULL)
        {
            ...
            level.disconnectStats[dc].rounds = cl->sess.stats.rounds;
            level.disconnectStats[dc].kills = cl->sess.stats.kills;
            level.disconnectStats[dc].deaths = cl->sess.stats.deaths;
            level.disconnectStats[dc].gibs = cl->sess.stats.gibs;
            level.disconnectStats[dc].suicides = cl->sess.stats.suicides;
            level.disconnectStats[dc].team_kills = cl->sess.stats.team_kills;
            level.disconnectStats[dc].headshots = cl->sess.stats.headshots;
            level.disconnectStats[dc].damage_given = cl->sess.stats.damage_given;
            level.disconnectStats[dc].damage_received = cl->sess.stats.damage_received;
            level.disconnectStats[dc].team_damage = cl->sess.stats.team_damage;
            level.disconnectStats[dc].acc_hits = cl->sess.stats.acc_hits;
            level.disconnectStats[dc].acc_shots = cl->sess.stats.acc_shots;
            level.disconnectStats[dc].efficiency = eff;
            level.disconnectStats[dc].revives = cl->sess.stats.revives;
            level.disconnectStats[dc].ammo_given = cl->sess.stats.ammo_given;
            level.disconnectStats[dc].med_given = cl->sess.stats.med_given;
            level.disconnectStats[dc].knifeKills = cl->sess.stats.knifeKills;
            level.disconnectStats[dc].killPeak = cl->sess.stats.killPeak;
            level.disconnectStats[dc].score = cl->ps.persistant[PERS_SCORE];
            level.disconnectStats[dc].dyn_planted = cl->sess.stats.dyn_planted;
            level.disconnectStats[dc].dyn_defused = cl->sess.stats.dyn_defused;
            level.disconnectStats[dc].obj_captured = cl->sess.stats.obj_captured;
            level.disconnectStats[dc].obj_destroyed = cl->sess.stats.obj_destroyed;
            level.disconnectStats[dc].obj_returned = cl->sess.stats.obj_returned;
            level.disconnectStats[dc].obj_taken = cl->sess.stats.obj_taken;
            ...
            {
                if (cl->sess.stats.aWeaponStats[m].atts || cl->sess.stats.aWeaponStats[m].hits || cl->sess.stats.aWeaponStats[m].deaths)
                {
                    level.disconnectStats[dc].aWeaponStats[m].kills = cl->sess.stats.aWeaponStats[m].kills;
                    level.disconnectStats[dc].aWeaponStats[m].deaths = cl->sess.stats.aWeaponStats[m].deaths;
                    level.disconnectStats[dc].aWeaponStats[m].headshots = cl->sess.stats.aWeaponStats[m].headshots;
                    level.disconnectStats[dc].aWeaponStats[m].hits = cl->sess.stats.aWeaponStats[m].hits;
                    level.disconnectStats[dc].aWeaponStats[m].atts = cl->sess.stats.aWeaponStats[m].atts;
                }
            }
    ...
    else
    {
        ...
        for (int i = 0; i < level.numPlayingClients; i++)
        {
            ...
            level.playerStats[i].rounds = cl->sess.stats.rounds;
            level.playerStats[i].kills = cl->sess.stats.kills;
            level.playerStats[i].deaths = cl->sess.stats.deaths;
            level.playerStats[i].gibs = cl->sess.stats.gibs;
            level.playerStats[i].suicides = cl->sess.stats.suicides;
            level.playerStats[i].team_kills = cl->sess.stats.team_kills;
            level.playerStats[i].headshots = cl->sess.stats.headshots;
            level.playerStats[i].damage_given = cl->sess.stats.damage_given;
            level.playerStats[i].damage_received = cl->sess.stats.damage_received;
            level.playerStats[i].team_damage = cl->sess.stats.team_damage;
            level.playerStats[i].acc_hits = cl->sess.stats.acc_hits;
            level.playerStats[i].acc_shots = cl->sess.stats.acc_shots;
            level.playerStats[i].efficiency = eff;
            level.playerStats[i].revives = cl->sess.stats.revives;
            level.playerStats[i].ammo_given = cl->sess.stats.ammo_given;
            level.playerStats[i].med_given = cl->sess.stats.med_given;
            level.playerStats[i].knifeKills = cl->sess.stats.knifeKills;
            level.playerStats[i].killPeak = cl->sess.stats.killPeak;
            level.playerStats[i].score = cl->ps.persistant[PERS_SCORE];
            level.playerStats[i].dyn_planted = cl->sess.stats.dyn_planted;
            level.playerStats[i].dyn_defused = cl->sess.stats.dyn_defused;
            level.playerStats[i].obj_captured = cl->sess.stats.obj_captured;
            level.playerStats[i].obj_destroyed = cl->sess.stats.obj_destroyed;
            level.playerStats[i].obj_returned = cl->sess.stats.obj_returned;
            level.playerStats[i].obj_taken = cl->sess.stats.obj_taken;
            ...
            for (m = WS_KNIFE; m < WS_MAX; m++)
            {
                if (cl->sess.stats.aWeaponStats[m].atts || cl->sess.stats.aWeaponStats[m].hits || cl->sess.stats.aWeaponStats[m].deaths)
                {
                    level.playerStats[i].aWeaponStats[m].kills = cl->sess.stats.aWeaponStats[m].kills;
                    level.playerStats[i].aWeaponStats[m].deaths = cl->sess.stats.aWeaponStats[m].deaths;
                    level.playerStats[i].aWeaponStats[m].headshots = cl->sess.stats.aWeaponStats[m].headshots;
                    level.playerStats[i].aWeaponStats[m].hits = cl->sess.stats.aWeaponStats[m].hits;
                    level.playerStats[i].aWeaponStats[m].atts = cl->sess.stats.aWeaponStats[m].atts;
                }
            }
        }
    }
...
int getPstats(json_t *jsonData, char *id, gclient_t *client) {
    ...
    */
    ...
    if(json_is_integer(pitem)) {
        client->sess.stats.kills=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "deaths");
    if(json_is_integer(pitem)) {
        client->sess.stats.deaths=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "gibs");
    if(json_is_integer(pitem)) {
        client->sess.stats.gibs=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "suicides");
    if(json_is_integer(pitem)) {
        client->sess.stats.suicides=json_integer_value(pitem);

    }

    pitem = json_object_get(pcat, "teamkills");
    if(json_is_integer(pitem)) {
        client->sess.stats.team_kills=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "damagegiven");
    if(json_is_integer(pitem)) {
        client->sess.stats.damage_given=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "damagereceived");
    if(json_is_integer(pitem)) {
        client->sess.stats.damage_received=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "damageteam");
    if(json_is_integer(pitem)) {
        client->sess.stats.team_damage=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "hits");
    if(json_is_integer(pitem)) {
        client->sess.stats.acc_hits=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "shots");
    if(json_is_integer(pitem)) {
        client->sess.stats.acc_shots=json_integer_value(pitem);
    ...
    */
    ...
    if(json_is_integer(pitem)) {
            client->sess.stats.revives=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "ammogiven");
    if(json_is_integer(pitem)) {
        client->sess.stats.ammo_given=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "healthgiven");
    if(json_is_integer(pitem)) {
        client->sess.stats.med_given=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "knifekills");
    if(json_is_integer(pitem)) {
        client->sess.stats.knifeKills=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "killpeak");
    if(json_is_integer(pitem)) {
        client->sess.stats.killPeak=json_integer_value(pitem);
    ...
    if(json_is_integer(pitem)) {
        client->sess.stats.dyn_planted=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "dyn_defused");
    if(json_is_integer(pitem)) {
        client->sess.stats.dyn_defused=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_captured");
    if(json_is_integer(pitem)) {
        client->sess.stats.obj_captured=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_destroyed");
    if(json_is_integer(pitem)) {
        client->sess.stats.obj_destroyed=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_returned");
    if(json_is_integer(pitem)) {
        client->sess.stats.obj_returned=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_taken");
    if(json_is_integer(pitem)) {
        client->sess.stats.obj_taken=json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_checkpoint");
    if (json_is_integer(pitem)) {
        client->sess.stats.obj_checkpoint = json_integer_value(pitem);


    }
    pitem = json_object_get(pcat, "obj_killcarrier");
    if (json_is_integer(pitem)) {
        client->sess.stats.obj_killcarrier = json_integer_value(pitem);
...
int G_read_round_jstats_reconnect(gclient_t* client)
{
    ...
    for (int i = 0; i < ARRAY_LEN(level.disconnectStats); i++)
    {
        sprintf(pGUID, "%s", client->sess.guid);

        if (!Q_stricmp(level.disconnectStats[i].guid, pGUID))
        {
            client->sess.stats.deaths = level.disconnectStats[i].deaths;
            client->sess.stats.kills = level.disconnectStats[i].kills;
            client->sess.stats.damage_given = level.disconnectStats[i].damage_given;
            client->sess.stats.damage_received = level.disconnectStats[i].damage_received;
            client->sess.stats.team_damage = level.disconnectStats[i].team_damage;
            client->sess.stats.team_kills = level.disconnectStats[i].team_kills;
            client->sess.stats.gibs = level.disconnectStats[i].gibs;
            client->sess.stats.acc_shots = level.disconnectStats[i].acc_shots;
            client->sess.stats.acc_hits = level.disconnectStats[i].acc_hits;
            client->sess.stats.headshots = level.disconnectStats[i].headshots;
            client->sess.stats.suicides = level.disconnectStats[i].suicides;
            client->sess.stats.med_given = level.disconnectStats[i].med_given;
            client->sess.stats.ammo_given = level.disconnectStats[i].ammo_given;
            client->sess.stats.revives = level.disconnectStats[i].revives;
            client->sess.stats.knifeKills = level.disconnectStats[i].knifeKills;
            client->sess.stats.dyn_planted = level.disconnectStats[i].dyn_planted;
            client->sess.stats.dyn_defused = level.disconnectStats[i].dyn_defused;
            client->sess.stats.obj_captured = level.disconnectStats[i].obj_captured;
            client->sess.stats.obj_destroyed = level.disconnectStats[i].obj_destroyed;
            client->sess.stats.obj_returned = level.disconnectStats[i].obj_returned;
            client->sess.stats.obj_taken = level.disconnectStats[i].obj_taken;
            ...
            for (int m = WS_KNIFE; m < WS_MAX; m++)
            {
                ...
                {
                    client->sess.stats.aWeaponStats[m].kills = level.disconnectStats[i].aWeaponStats[m].kills;
                    client->sess.stats.aWeaponStats[m].deaths = level.disconnectStats[i].aWeaponStats[m].deaths;
                    client->sess.stats.aWeaponStats[m].headshots = level.disconnectStats[i].aWeaponStats[m].headshots;
                    client->sess.stats.aWeaponStats[m].hits = level.disconnectStats[i].aWeaponStats[m].hits;
                    client->sess.stats.aWeaponStats[m].atts = level.disconnectStats[i].aWeaponStats[m].atts;
                }
            }
...
void G_jWeaponStats(void) {
    ...
    for (j = 0; j < ARRAY_LEN(level.playerStats); j++)
    {
...
void G_jstatsByPlayers(qboolean wstats, qboolean clientDisconnected, gclient_t *client) {
    ...
    if (!clientDisconnected)
    {
        ...
        for (int j = 0; j < ARRAY_LEN(level.playerStats); j++)
        {
            if (level.playerStats[j].guid == NULL) // if array index is empty continue
                continue;
	...
	for ( i = TEAM_RED; i <= TEAM_BLUE; i++ ) {
        ...
        for ( j = 0; j < level.numPlayingClients; j++ ) {
          ...
          //  json_object_set_new(jdata, "team", json_string((i == TEAM_RED) ? "Axis" : "Allied"  ));
            ...
            json_object_set_new(jdata, "num_rounds", json_integer(cl->sess.stats.rounds));

            jcat = json_object();

            json_object_set_new(jcat, "kills", json_integer(cl->sess.stats.kills));
            json_object_set_new(jcat, "deaths", json_integer(cl->sess.stats.deaths));
            json_object_set_new(jcat, "gibs", json_integer(cl->sess.stats.gibs));
            json_object_set_new(jcat, "suicides", json_integer(cl->sess.stats.suicides));
            json_object_set_new(jcat, "teamkills", json_integer(cl->sess.stats.team_kills));
            json_object_set_new(jcat, "headshots", json_integer(cl->sess.stats.headshots));
            json_object_set_new(jcat, "damagegiven", json_integer(cl->sess.stats.damage_given));
            json_object_set_new(jcat, "damagereceived", json_integer(cl->sess.stats.damage_received));
            json_object_set_new(jcat, "damageteam", json_integer(cl->sess.stats.team_damage));
            json_object_set_new(jcat, "hits", json_integer(cl->sess.stats.acc_hits));
            json_object_set_new(jcat, "shots", json_integer(cl->sess.stats.acc_shots));
            json_object_set_new(jcat, "accuracy", json_real(((cl->sess.stats.acc_shots == 0) ? 0.00 : ((float)cl->sess.stats.acc_hits / (float)cl->sess.stats.acc_shots) * 100.00f)));
            json_object_set_new(jcat, "revives", json_integer(cl->sess.stats.revives));
            json_object_set_new(jcat, "ammogiven", json_integer(cl->sess.stats.ammo_given));
            json_object_set_new(jcat, "healthgiven", json_integer(cl->sess.stats.med_given));
            json_object_set_new(jcat, "knifekills", json_integer(cl->sess.stats.knifeKills));
            json_object_set_new(jcat, "killpeak", json_integer(cl->sess.stats.killPeak));
            ...
            json_object_set_new(jcat, "dyn_planted", json_integer(cl->sess.stats.dyn_planted));
            json_object_set_new(jcat, "dyn_defused", json_integer(cl->sess.stats.dyn_defused));
            json_object_set_new(jcat, "obj_captured", json_integer(cl->sess.stats.obj_captured));
            json_object_set_new(jcat, "obj_destroyed", json_integer(cl->sess.stats.obj_destroyed));
            json_object_set_new(jcat, "obj_returned", json_integer(cl->sess.stats.obj_returned));
            json_object_set_new(jcat, "obj_taken", json_integer(cl->sess.stats.obj_taken));
            ...
            for (m = WS_KNIFE; m < WS_MAX; m++) {
                if (cl->sess.stats.aWeaponStats[m].atts || cl->sess.stats.aWeaponStats[m].hits ||
                    cl->sess.stats.aWeaponStats[m].deaths) {
                        weapOb = json_object();
                        json_object_set_new(weapOb, "weapon", json_string((m >= WS_KNIFE && m < WS_MAX ) ? aWeaponInfo[m].pszName : "UNKNOWN" ));
                        json_object_set_new(weapOb, "kills", json_integer(cl->sess.stats.aWeaponStats[m].kills));
                        json_object_set_new(weapOb, "deaths", json_integer(cl->sess.stats.aWeaponStats[m].deaths));
                        json_object_set_new(weapOb, "headshots", json_integer(cl->sess.stats.aWeaponStats[m].headshots));
                        json_object_set_new(weapOb, "hits", json_integer(cl->sess.stats.aWeaponStats[m].hits));
                        json_object_set_new(weapOb, "shots", json_integer(cl->sess.stats.aWeaponStats[m].atts));
                        json_array_append(weapArray, weapOb);
                        json_decref(weapOb);
                }
            }
...
void G_writeGeneralEvent (gentity_t* agent,gentity_t* other, char* weapon, int eventType) {
    ...
    switch ( eventType ) {
	    ...
	    case teamFirstSpawn:
            ...
            int redRespawnTime = (g_redlimbotime.integer - level.redReinfOffset) / 1000;
            int blueRespawnTime = (g_bluelimbotime.integer - level.blueReinfOffset) / 1000;
