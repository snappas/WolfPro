...
//WolfPro: the vendored cimgui.h (generated from Dear ImGui 1.91.8) does not emit the
//"_c" suffixed lightweight mirror types (ImVec2_c/ImVec4_c/ImRect_c) that newer cimplot
//generator output expects to already exist. They are plain aliases of the real
//ImVec2/ImVec4/ImRect types, which are visible here either via cimgui.h itself (pure C
//mode, CIMGUI_DEFINE_ENUMS_AND_STRUCTS defined) or via the real Dear ImGui headers
//already included by implot.h/implot_internal.h before this file (C++ mode, as used by
//cimplot.cpp).
typedef ImVec2 ImVec2_c;
typedef ImVec4 ImVec4_c;
typedef ImRect ImRect_c;

//WolfPro: several generated declarations below return ImVec2/ImVec4 by value
//from an extern "C" function -- MSVC warns about this (C4190) since those
//types aren't C-compatible, matching the exact same warning cimgui.h already
//disables (5287) for its own vendored/generated content. None of these
//specific functions are called from any C translation unit in this codebase
//(the ones we do call from C -- GetPlotMousePos/PlotToPixels/GetPlotPos/
//GetPlotSize -- have dedicated out-pointer-safe wrappers in
//cimplot_wolfpro_safe.h/.cpp instead, since calling THESE by-value forms
//from C is the actual hazard C4190 is warning about); this only silences the
//warning for the remaining, C++-only-called declarations in this vendored file.
#ifdef _MSC_VER
...
//WolfPro: cimplot.h's generator unconditionally targeted implot's ImTextureRef-based
//PlotImage overload, but implot.h itself guards that behind IMGUI_HAS_TEXTURES and falls
//back to the classic ImTextureID overload for pre-texture-refactor Dear ImGui (as
//vendored here, 1.91.8). Mirror that same guard here rather than requiring a type
//(ImTextureRef) that doesn't exist in this ImGui version.
...
#else
CIMGUI_API void ImPlot_PlotImage(const char* label_id,ImTextureID tex_ref,const ImPlotPoint_c bounds_min,const ImPlotPoint_c bounds_max,const ImVec2_c uv0,const ImVec2_c uv1,const ImVec4_c tint_col,const ImPlotSpec_c spec);
#endif
