...
//WolfPro: ImTextureRef only exists in Dear ImGui's texture-refactor versions; the
//vendored 1.91.8 predates it (IMGUI_HAS_TEXTURES is never defined here), so these
//converters are dead code under this build and guarded out. See matching guard on
//ImPlot_PlotImage below and in cimplot.h.
...
{
    return ImPlot::PlotImage(label_id,tex_ref,ConvertToCPP_ImPlotPoint(bounds_min),ConvertToCPP_ImPlotPoint(bounds_max),ConvertToCPP_ImVec2(uv0),ConvertToCPP_ImVec2(uv1),ConvertToCPP_ImVec4(tint_col),ConvertToCPP_ImPlotSpec(spec));
}
#endif
