...
qboolean CMod_LoadCustomEntityString(const char* name) {
	...
	if (file && entFileLen > 0)
	{
		...
		Com_Printf(va("Loaded custom entities from %s\n", filename));
		return qtrue;
	}
...
void CM_LoadMap( const char *name, qboolean clientload, int *checksum ) {
	...
	*checksum = last_checksum;
	...
	if ( header.version != BSP_VERSION && header.version != BSP_VERSION - 1 ) {
	...
	CMod_LoadEntityString( &header.lumps[LUMP_ENTITIES], name ); //custom entities
