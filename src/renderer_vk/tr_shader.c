...
static void ParseSkyParms( char **text ) {
	...
	if ( strcmp( token, "-" ) ) {
		for ( i = 0 ; i < 6 ; i++ ) {
			...
			shader.sky.outerbox[i] = R_FindImageFile( ( char * ) pathname, qfalse, qfalse, GL_CLAMP );
	...
	if ( strcmp( token, "-" ) ) {
		for ( i = 0 ; i < 6 ; i++ ) {
			...
			// sky box faces are always full resolution and unmipped: independently-generated
			// mips per face don't match at the edges and mipmapping is a classic cube-seam cause
			shader.sky.innerbox[i] = R_FindImageFile( ( char * ) pathname, qfalse, qfalse, GL_CLAMP );
...
static shader_t *GeneratePermanentShader( void ) {
	...
	*newShader = shader;
	...
	if ( newShader->isSky ) {
		RB_CreateSkyBoxStages( newShader );
	}
